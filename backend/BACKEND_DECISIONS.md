...
Per ogni scelta non banale del backend: cosa è stato deciso, dove, e **3 ipotesi sul perché** (una tecnica, una pragmatica, una di vincolo/convenzione). Le scelte che la review ha modificato hanno una nota **Stato post-review** che racconta cosa è cambiato e perché.
...
La presenza esplicita di `await nasa_client.shutdown()` post-yield lascia già intendere che volevo un teardown affidabile del client HTTP. Quindi direi che il POV tecnico comandi su tutto.
...
Anche qui la principale è quella tecncia, Il wrapper sarebbe puro overhead se non servisse alla testabilità.
...
Questa volta comanda la pragmaticità. Volevo tenere un footprint minimo.
...
La coesistenza di `/api/health` (visibile) e `/health` (out-of-schema) è la firma della compatibilità con probe esterni. Per cui motivo tecnico.
...
Tecnica + convenzione direi, un pattern tipico "prod fissa + dev/preview dinamiche".
...
Nessuna route ha try/except: il pattern è "raise nelle service, catch globale". Per cui molto pratica la decisione.
...
L'ho fatto perché richiesto dalla challenge, per cui direi 3 se dovessi sceglierne una.
...
Tecnica, con la combinazione con `asyncio.Semaphore(upstream_concurrency)` è evidente (spero) una progettazione consapevole di concorrenza/thundering herd.
...
Tecnica principalmente, l'`__init__` keyword-only di `NeoService` è la firma del DI test-friendly.
...
Sicuramente non convenzione, ma tecnica e pragmatica. La logica differenziata 429 vs 5xx vs timeout è una scelta informata sul comportamento di NASA NeoWs, non un template generico.
...
Scelta pragmatica e coerente con la natura demo/take-home.
...
Il chunking nasce dal vincolo upstream.
...
Superficie API ridotta (Tecnica).
...
Molto pratica. Pydantic 1.10 va in EOL/security-only nel 2025-2026: vale la pena pianificare la migrazione a v2 + `pydantic-settings` prima che diventi obbligatoria. Farò un issue per il futuro.
...
Una volta usata env per il secret, conviene uniformare tutto il resto.
...
La scelta nasce da una valutazione esplicita di sicurezza durante la review: la richiesta dell'utente è stata "Funziona rimuovere l'endpoint dalla superficie pubblica? Se funziona mi sembra più sicura la seconda opzione".
